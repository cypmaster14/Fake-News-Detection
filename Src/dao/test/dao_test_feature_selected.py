...
import pandas as pd
import numpy as np
from Src.entities.test_feature_selected import TestFeatureSelected


class TestFeatureSelectedDAO(object):
    ...
    test_feature_selected_file_location = "../Test_feature_selected.csv"
    ...
    def insert_all(self):
        ...
        for test in test_data:
            ...
            test_entity = TestFeatureSelected(
                test[0][0],
                test[0][1],
                test[0][2],
                test[0][3],
    ...
    def find_all(self):
        ...
        query = [self.get_as_list(entry) for entry in session.query(TestFeatureSelected).all()]
        test_feature_selected = pd.DataFrame(query, columns=self.columns_header)
        print(len(test_feature_selected))

        session.close()
        return test_feature_selected

    def get_as_list(self, test_feature_selected: TestFeatureSelected) -> list:
...
if __name__ == "__main__":
    test_feature_selected_dao = TestFeatureSelectedDAO()
    test_feature_selected_dao.insert_all()
    test_feature_selected_dao.find_all()
